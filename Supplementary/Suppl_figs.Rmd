...
sparsity_plot <- 
  ...
  ggplot(aes(x = Technology, y = Percent_NonZero, group = Datatype, 
             fill = Datatype)) +
...
guttube_clusters <- c(17, 22)
neuralcrest_clusters <- c(14, 21)
spinalcord_clusters <- c(9, 19)
```
...
clustToplot <- ifelse(spe_me$ClustSIGNAL %in% neuralcrest_clusters, 
                      spe_me$ClustSIGNAL, "Others")
...
spe_me_sub <- spe_me[, spe_me$ClustSIGNAL %in% neuralcrest_clusters]
markers <- findMarkers(spe_me_sub, test.type = "t", 
                       groups = as.character(spe_me_sub$ClustSIGNAL))
...
```{r}
#### panel e - spatial plots of Spinal cord region and clusters
...
# combining spatial plots
comb_spinal_plot <- spinal_annot / spinal_clust + plot_layout(guides = "collect")
```

```{r}
#### panel f - top markers of Spinal cord clusters
...
```{r}
#### Suppl Fig. 37 - inhibitory neurons 
...
```{r}
#### Suppl Fig. 38 - excitatory neurons
