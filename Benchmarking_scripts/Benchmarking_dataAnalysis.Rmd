...
spagcn_res_me <- runSpaGCN(spe = spe_me, 
                           sample_label = "embryo", 
                           n_clusters = 23, SEED = 567, 
                           py_file = py_file,
                           python_path  = py_path)
# spagcn_res_me$raw[[2]]$mem
saveRDS(spagcn_res_me, file = "./data_outputs/benchmarkOut/res_me_spagcn.Rds")
```

```{r}
#### STAGATE - modified the code to input kNN instead of radius cut-off.
.libPaths("/enna/nobackup/biostat/Rpackages/v4")

library(SpatialExperiment)

datapath = "/dski/nobackup/biostat/datasets/SpatialTranscriptomics/"
spe_me <- readRDS(paste0(datapath,  "seqFISH_SpatialMouseAtlas_mouseEmbryos/spe_mouseEmbryo.Rds"))
# remove low quality cells
spe_me <- spe_me[, spe_me$celltype_mapped_refined != "Low quality"] 
spe_me$celltype_mapped_refined <- factor(spe_me$celltype_mapped_refined)
# spe <- spe_me[, spe_me$embryo == "embryo1"]
